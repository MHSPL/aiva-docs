...
??? question "What are storage slots?"
    Storage slots determine how many tables and BAM files your account can hold. Each uploaded sample table uses 1 slot. BAM files generated by [secondary analysis](samples/secondary-analysis.md) pipelines also use 1 slot each. For example, a Parabricks germline pipeline uses 2 slots (1 table + 1 BAM), and a somatic paired pipeline uses 3 slots (1 table + 2 BAMs). Delete samples you no longer need to free up slots. See [Storage slots](getting-started/subscription-tiers.md#storage-slots) for limits by tier.
