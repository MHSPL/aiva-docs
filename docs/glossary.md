...
:   A binary format for storing sequence alignment data. BAM files are generated by [secondary analysis](samples/secondary-analysis.md) pipelines in AIVA and count toward your account's [storage slots](getting-started/subscription-tiers.md#storage-slots).
...
**Storage Slot**
:   A unit of account storage capacity. Each sample table and each BAM file consumes one storage slot. Slot limits vary by [subscription tier](getting-started/subscription-tiers.md#storage-slots).
